...
def hacer_request_api(Edad, F_Consumo_verduras, N_comidas, F_actvidad_fisica, Consumo_agua):
    request_data = [{
        "Edad": Edad,
...
def main():
    ...
    if choice == 'Datos':
        ...
        col1.title('')
        col2.subheader(
            '¿Cual es el promedio de litros de agua consumida por categorías de obesidad?')
        grafF(col2)
        col2.write("""
            En la grafica se observa que el consumo de agua por litro no afecta el hecho de pertenecer a una mayor o menor categoría de obesidad. 
        ...
        boton_predecir = c3.button('Predecir')

        if boton_predecir:
            prediccion = hacer_request_api(Edad, F_consumo_verduras, N_comidas, F_actvidad_fisica, Consumo_agua)
